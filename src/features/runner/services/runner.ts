...
import { RUNNER_WORKING_DIRECTORY } from "./context"
import { fileExists } from "../discovery/shared"
...
export async function resolveRunnerProjectContext(
  ...
  if (requestedCommands.length || (await fileExists(resolve(requestedRoot, ".git")))) {
    return { root: requestedRoot, commands: requestedCommands }
  }

  let candidate = dirname(requestedRoot)
  while (candidate !== dirname(candidate)) {
    if (await fileExists(resolve(candidate, ".git"))) {
      return {
        root: candidate,
