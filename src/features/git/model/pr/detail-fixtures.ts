...
export function demoPullRequestDetails(item: PullRequestSummary): PullRequestDetails {
  ...
  return {
    ...
    comments: [
      {
        ...
        reactionGroups: [{ content: "THUMBS_UP", count: 2, viewerHasReacted: true }],
      },
      {
        id: `${item.identity.nodeId}-comment-2`,
        author: { login: "ana" },
        body: `↳ ${item.identity.url}#issuecomment-1\n\n@bia Resposta fictícia agrupada sob o comentário.`,
        createdAt: new Date(new Date(item.updatedAt).getTime() + 1_000).toISOString(),
    ...
    pages: {
      ...
      files: COMPLETE_PAGE,
      comments: { ...COMPLETE_PAGE, totalCount: 2 },
