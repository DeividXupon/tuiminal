...
describe("UI settings", () => {
  ...
  test.each([undefined, "Ctrl+C"])(
    "migrates old settings and an invalid Master Key (%s) while preserving live colors",
    (terminalMasterKey) => {
      ...
      `
        ...
        expect(JSON.parse(readFileSync(settingsPath, "utf8")).terminalMasterKey).toBe("Ctrl+A")
        expect(JSON.parse(readFileSync(settingsPath, "utf8")).terminalAgentCommands).toEqual([
          "acme",
          "team.assistant",
        ])
