...
test.each(["framed", "compact"] as const)(
  ...
  async (layout) => {
    ...
    for (const height of [30, 28, 32, 34, 36, 38, 30]) {
      ...
      expect(tui.captureCharFrame()).toContain("[V] TLS: verificar")
    }
    await key("c")
    await key("v")
    ...
    expect(tui.captureCharFrame()).toContain("[V] TLS INSEGURO")
...
test.each(["framed", "compact"] as const)(
  ...
  async (layout) => {
    ...
    await key("ESCAPE")
    await key("o")
    await settle(() => tui?.captureCharFrame().includes("[1] Opções") ?? false)
    const bounds = optionsBounds()
