...
- Real interactive launches begin with a short responsive brand animation: the four `#4B75FF` logo blocks fall and settle one at a time, then the Tuiminal wordmark appears. Keep tools unmounted until it completes, allow `[Enter]`/`[Esc]` or a mouse click to skip it, and use the compact mark on constrained terminals. Headless HTTP commands never mount it; automated workspace tests may explicitly bypass it.
