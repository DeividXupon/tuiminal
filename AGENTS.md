...
- In Diffs, `[Tab]`, `[H/L]`, and `[←/→]` move focus between the file tree and diff, `[O]` opens Log, and `[V]` visibly identifies and cycles the diff layout. Keep the fixed shortcut footer mounted above scrollable content even during refreshes, actions, messages, and errors.
