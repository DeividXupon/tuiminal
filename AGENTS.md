...
- Treat filenames from Git status as literal paths in single-file diff, stage, and unstage commands, including the unborn-HEAD fallback. `--` separates options but still permits Git pathspec wildcards and magic; use `--literal-pathspecs` before the subcommand so selecting one file never includes matching neighbors.
...
- Enforce autostart at the configuration source boundary: imported mprocs commands remain manual even when their own file contains `autostart: true`; their restart policy still applies after a manual start.
...
- Prepared HTTP requests retain the names of sensitive headers, including custom API-key auth, explicitly sensitive entries, and resolved private values. Redirects preserve these headers only within the original origin; after a host, port, or scheme change they remain removed for the rest of that redirect chain, including a return to the original origin. Keep the standard credential-header fallback for directly prepared requests.
- Header sensitivity follows explicit field metadata and template references, recursively including public aliases of private variables. Do not classify public headers by substring matches against unrelated secret values: short unused secrets must not remove version, content-type, or other public headers during redirects.
- Sensitive HTTP Path rows participate in the same secret-value set as headers, query, and body fields, including disabled rows that remain serializable. Redact their resolved values from preview, cURL, conflict views, report URLs, assertion display strings, and errors; evaluate assertions before redaction and use URL-aware redaction for percent-encoded paths. Reject literal sensitive Path values when saving public `.http` files, while preserving private references and the original transport values.
...
- Writable SQL results require direct base-column projections or wildcards; a returned name matching a schema column is not proof of provenance. Expressions, renamed columns, grouping, and `DISTINCT` remain read-only so computed/aliased primary keys cannot target a different record. Keep executable comments and dialect-dependent syntax out of this conservative eligibility check.
- Pass the active connection driver into SQL projection eligibility. Unqualified double-quoted projections are ambiguous strings in MySQL unless `ANSI_QUOTES` is known; keep them read-only for MySQL or an unknown driver without guessing session SQL mode. PostgreSQL/SQLite double-quoted columns, qualified references, backticks, and unchanged output aliases retain their normal eligibility.
