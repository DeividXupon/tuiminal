...
- PR, Issues, and Inbox share the same missing/outdated GitHub CLI requirement panel. It explains `gh`, detects a fixed allowlisted package-manager command, displays it before execution, and starts its interactive PTY only after `[I]` or a mouse click. Never interpolate remote/user text into that command or capture/persist PTY input. On a successful exit, require `gh >= 2.40.0` before reloading the Git view; unmount stops only the owned installer process group. GitHub authentication remains a separate explicit `gh auth login` step.
