...
Painéis ao lado do Tuiminal na mesma sessão tmux são descobertos automaticamente.
O espelho carrega a tela que já existe e continua seguindo
...
- **Novo terminal** sempre abre uma seção separada. Para dividir a seção atual, use a Master Key seguida de `[V]` (lado) ou `[H]` (abaixo).
- A lista de ações da Master Key usa títulos na cor primária adequada ao tema e tags coloridas alinhadas à direita para recursos, capacidades exclusivas de agentes, operações de terminal, controles da sidebar, navegação e ações do app. `[C]`, `[R]` e `[G]` não fazem parte desse menu.
- Use a Master Key `[M]` para escolher entre a sidebar e os boxes visíveis de terminal, histórico de mensagens enviadas e Live Diff. O box que tinha foco fica destacado em azul, as outras opções ficam levemente escurecidas e a instrução central usa um fundo próprio adaptado ao tema; navegue com as setas ou `[H/J/K/L]` e pressione `[Enter]` para focá-lo.
- Use o botão **Comando** da sidebar para iniciar um comando personalizado em uma nova seção.
- Use a Master Key `[A]` ou **Novo Codex** para abrir a interface completa do Codex em uma nova seção. O Tuiminal inicia um app-server em localhost, conecta a interface do Codex a ele e lê os eventos públicos para mostrar a atividade do agente. Digite a tarefa no Codex; as aprovações continuam na interface dele.
- Em `[,]` → **Conexão remota**, confira o resumo, pressione `[Enter]` para focar o formulário, salve os dados da conexão SSH, escolha o único perfil ativo com `[A]` e teste o acesso por chave à VPS. **Verificar servidor** confere o GitHub via SSH e se o Codex CLI está instalado e autenticado. **Configurar servidor** abre um terminal SSH interativo com um tutorial passo a passo embaixo; você executa cada comando, e **Confirmar configuração** testa novamente a barreira atual antes de avançar. O Tuiminal não cria chaves, instala ferramentas, conecta contas, copia projetos nem inicia um agente remoto automaticamente. **Novo Codex** continua local nesta versão.
...
- Acompanhar agentes reconhecidos exclusivamente em **Agentes**, separados entre **Local • term** para agentes observados por terminais nativos, tmux ou externos e **Local • localhost** para sessões integradas do app-server abertas com a Master Key `[A]`. Os agentes de terminal aparecem primeiro, com um loader animado enquanto trabalham. As linhas do Codex integrado ganham uma terceira linha para atividade de código, comando, plano e ferramenta; o ícone atual pulsa conforme os eventos públicos do app-server. Os ícones compatíveis `{}`, `>_`, `txt` e `●` são sempre usados, sem exigir uma fonte modificada. Quando o Codex está parado, o estado da primeira linha o representa e todos os ícones de atividade permanecem inativos. **Terminais** mostra os demais terminais. As pastas e divisões originais são preservadas, e o terminal volta para Terminais quando o agente encerra. Com a Master Key aberta, os primeiros nove agentes e terminais visíveis mostram teclas sequenciais `[1]`, `[2]`, … `[9]` na mesma ordem terminal-localhost; digite uma delas para ativá-lo.
- Abra abaixo do terminal o histórico de mensagens enviadas de uma sessão integrada do Codex com a Master Key seguida de `[S]`. As colunas seguem a ordem tempo decorrido, status, mensagem, imagem, áudio, skill e modelo. O status combina a duração do turno com `✓` para conclusão, `×` para falha ou interrupção e `…` durante a execução, como `18m ✓`. Imagem, áudio e skill mostram `✓` somente quando aquela entrada foi enviada. A configuração aparece como `gpt-6-sol · medium · fast`; turnos históricos usam como fallback a configuração atual da thread selecionada quando o app-server não a preservou por turno e mostram `—` somente quando nenhuma das duas fontes está disponível. Use `[J/K]`, `[↑/↓]` ou o mouse para selecionar uma linha e `[Enter]` para abrir os detalhes. O detalhe expandido usa metade da coluna do terminal e preenche essa área com cards de mensagem, resposta, atividade e alterações adaptados ao tema. Ele oferece `[M]` mensagem, `[R]` resposta final e outros textos públicos do agente, `[A]` atividade pública completa e `[D]` alterações daquele turno. A tela de alterações separa o patch específico do prompt em blocos por arquivo, cada um com caminho visível, tipo da mudança, estatísticas, syntax highlighting, gutter com números de linha e fundos para adições e remoções; patches antigos incompletos usam linhas coloridas em vez de uma prévia vazia. `[Esc]` volta sucessivamente de seção, detalhe, lista e terminal; `[X]` fecha o painel. A visualização em memória combina novas entradas públicas do app-server com todo o histórico anterior, carregando todas as páginas da thread selecionada; trocar de thread substitui a tabela. Ela expõe respostas finais, comentários, resumos públicos de raciocínio, planos, atividades e alterações, mas nunca o conteúdo do raciocínio privado. É limpa ao reiniciar ou fechar até que a thread seja carregada novamente. Com o Live Diff aberto, ele continua ocupando toda a altura à direita.
- Em uma divisão com dois terminais Codex integrados, cada terminal pode manter seu próprio histórico de mensagens enviadas aberto; abrir ou fechar um não substitui o outro.
- Abra e foque o **Live Diff** para um agente reconhecido com a Master Key seguida de `[D]`; use o mesmo atalho novamente para restaurar o foco e pressione `[X]` enquanto estiver focado para fechá-lo. Ele usa o painel existente do agente e mostra o patch Git completo ao lado do terminal. O painel mantém a largura catorze colunas menor que sua divisão habitual; ao focar o código, só a prévia se alarga em 30 colunas, enquanto **Arquivos** e **Info** mantêm suas larguras. As linhas longas do diff não quebram até você focar o código com `[Enter]`; então passam a quebrar. No código, `[J/K]` ou `[↑/↓]` rolam uma linha, `[H/L]` ou `[←/→]` rolam meia página, e `[Esc]` volta à lista e ao acompanhamento automático. As linhas identificam alterações **New**, **Edit**, **Delete**, **Rename**, **Copy** e **Type**. Quando uma rodada de consulta detecta mudanças em um ou vários arquivos, cada linha afetada recebe um fundo azul que volta gradualmente ao normal em dois segundos. Na lista, `[J/K]` seleciona arquivos e `[H/L]` ou `[←/→]` destaca um projeto. `[N]` mostra ou oculta os arquivos desse projeto; o fundo vivo indica visível, e o fundo opaco indica oculto. O resumo de Info mostra o nome da pasta do último projeto alterado à direita dos totais, na mesma linha; o cabeçalho do Live Diff mostra `Show auto: true/false`. As caixas de Arquivos e Info não têm títulos. **Observando** lista os projetos como `/nome-da-pasta`, com fundos separados e linhas de continuação equilibradas e alinhadas à esquerda quando necessário. O Git continua sendo consultado para projetos ocultos. Pressione `[A]` na navegação do Live Diff ou clique em **Adicionar projeto** junto dos outros atalhos para abrir um seletor pesquisável de projetos Git próximos; worktrees relacionados são descobertos automaticamente. A prévia acompanha o arquivo visível mais recente e o último hunk alterado; selecionar um arquivo mais antigo pausa esse acompanhamento. `[Esc]` na lista devolve o foco ao terminal. O último estado permanece após o agente sair até o painel ser fechado.
- Em uma seção com dois terminais divididos, cada agente reconhecido pode manter seu próprio Live Diff aberto; abrir ou fechar o segundo não substitui o primeiro. O Live Diff normalmente usa cerca de metade do espaço do terminal selecionado, mantendo a outra metade e o terminal irmão visíveis. O diff empilhado usa toda a largura do pane, sem a reserva de catorze colunas do layout lado a lado. Ele cobre o espaço selecionado somente em layouts muito pequenos. Os dois processos permanecem montados, e `[Esc]` ou `[X]` restaura o terminal sem reiniciar nenhum deles.
- Fixar a sidebar ativa com a Master Key seguida de `[B]` para manter Terminais e Agentes visíveis, sem borda direita, ao navegar pelas ferramentas do Tuiminal. Use `[L]` no menu de ações para focá-la, percorra cabeçalhos de pastas, Terminais e Agentes continuamente com `[↑/↓]` ou `[J/K]` e pressione `[Enter]` para alternar uma pasta ou abrir o item destacado. Um light sweep rápido atravessa o fundo inteiro da barra quando ela recebe o foco. Dentro do tmux, o Tuiminal também mantém um split lateral esquerdo marcado em cada janela do servidor atual: depois de selecionar esse split pela navegação normal do tmux, seus controles diretos já funcionam, e ele continua clicável enquanto o terminal vizinho está focado. Recolher ou expandir uma pasta mantém esse split da sidebar selecionado. Selecionar um terminal ou agente em qualquer split fixado sempre retorna ao Tuiminal e o abre no workspace Terminal. Se esse painel ainda não estiver aberto ali, o Tuiminal cria seu espelho e o lista em Agentes ou Terminais conforme o caso. A Master Key desse split abre as mesmas ações e executa ações sem diálogo sem sair da janela. Soltar a barra remove apenas esses splits auxiliares e restaura a configuração anterior de mouse do tmux.
- Digitar `exit` em um shell interativo fecha o terminal e o remove de Terminais; comandos personalizados concluídos mantêm a saída final disponível para consulta e reinício.
- Usar nos painéis embutidos as cores ANSI e as cores padrão de texto e fundo informadas pelo terminal externo, preservando cores RGB explícitas. Preservar cores, cursor, saída e processos ao redimensionar, trocar de seção ou de ferramenta. Reinícios aguardam o encerramento do processo anterior; ao sair, o Tuiminal desconecta terminais tmux persistentes e encerra apenas processos nativos próprios.

Em `[,]` → **Terminal**, escolha a **Master Key** (padrão `[Ctrl+B]`). Ao pressioná-la,
abre um modal central com as ações em lista vertical e duas linhas por item: atalho
entre colchetes e nome, seguidos de uma descrição curta. O modal também mostra até
seis conversas da lista `/resume` do Codex local para o projeto atual. Pressione uma
tecla de ação ou clique no controle; selecione uma conversa para retomá-la na interface
oficial do Codex em uma nova seção. Pressione `[/]` para focar o filtro de ações e
conversas. `[Esc]` primeiro sai do filtro e depois fecha o modal, devolvendo o foco
ao terminal. Os atalhos globais da barra superior ficam em cinza até a Master Key ser
ativada; então ficam azuis junto das teclas temporárias da sidebar, sem deslocar o layout.
Repetir a Master Key envia essa tecla ao processo.

| Após a Master Key | Ação |
| --- | --- |
| `[N]` | Novo terminal em uma nova seção |
| `[A]` | Abrir a interface do Codex conectada ao app-server em nova seção |
| `[V]` / `[H]` | Escolher novo shell ou agente existente e dividir para o lado / abaixo |
| `[S]` | Abrir ou focar histórico de mensagens enviadas do Codex integrado |
| `[M]` | Escolher a sidebar ou outro box visível com setas ou `[H/J/K/L]` e focá-lo com `[Enter]` |
...
**Agentes** mostra todos os agentes em execução sob **Local • term** ou **Local • localhost**, com estado alinhado à direita e
...
Pedidos de ação e conclusões não vistas em segundo plano também geram uma
notificação; clique nela para abrir o painel exato ou dispense-a sem mudar o foco.
...
O Live Diff consulta o Git a cada 250 ms enquanto o agente está em execução, sem reiniciar
o terminal ou alterar os arquivos do projeto. Descobre projetos pelo diretório do terminal,
pelos diretórios de trabalho dos processos associados quando disponíveis e por worktrees
Git relacionados. É uma visão das mudanças nesses diretórios, não uma atribuição precisa
de cada edição ao agente. Em sistemas sem inspeção de diretórios dos processos, use a
adição manual para incluir projetos adicionais. Ao terminar o agente, o painel conserva
o último estado até ser fechado.
As linhas recém-detectadas mantêm o fundo azul enquanto permanecerem no patch, mesmo
quando um arquivo alterado só for aberto depois. As linhas azuis mantêm o brilho em
loop, edições posteriores somam outras linhas azuis e um espaço mínimo separa hunks adjacentes.

Os perfis locais reconhecem controles visíveis de Codex, Claude Code, Gemini,
OpenCode, Amp, Antigravity, Cline, GitHub Copilot, Cursor Agent, Devin, Droid,
Grok, Hermes, Kilo, Kimi, Kiro, Letta, Maki, Muse, Pi, Qoder e Qwen, além de
títulos de estado de alguns agentes. OMP e MastraCode são identificados, mas
não têm perfil local confiável de atividade. Não instala hooks nem
...
O estado recolhido ou expandido das pastas predefinidas e a pasta atribuída a cada painel tmux são salvos por
...
| `bun run benchmark` | Medir tempos de resposta locais das cinco ferramentas; veja o [guia de benchmarks](./docs/performance-benchmarks.md) |
| `bun run benchmark:all` | Reunir as suítes mantidas de serviços, interface nativa e inicialização em um relatório JSON |
| `bun run benchmark:database:drivers` | Medir SQLite, MySQL, MariaDB e PostgreSQL com containers Docker descartáveis |
