...
- Follow recognized agents exclusively in **Agents**, with an animated loader while they work. **Terminals** shows the remaining terminals. The original folders and splits are preserved, and a terminal returns to Terminals when its agent ends. With the Master Key open, the first nine visible agents and terminals show sequential `[1]`, `[2]`, … `[9]` labels; type one to activate it.
