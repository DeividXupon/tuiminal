...
- **New terminal** always opens a separate section. To split the current section, use the Master Key followed by `[V]` (right) or `[H]` (below).
- The Master Key action list keeps theme-aware primary titles and right-aligned colored tags for features, agent-only capabilities, terminal operations, sidebar controls, navigation, and app actions. `[C]`, `[R]`, and `[G]` are not actions in this menu.
- Use Master Key `[M]` to choose among the sidebar and the visible terminal, sent-message history, and Live Diff boxes. The previously focused box is highlighted in blue, every other choice is slightly darkened, and the central instruction has its own theme-aware background; move with arrows or `[H/J/K/L]` and press `[Enter]` to focus it.
- Use the sidebar's **Command** button to start a custom command in a new section.
- Use Master Key `[A]` or **New Codex** to open the complete Codex terminal interface in a new section. Tuiminal starts an app-server on localhost, connects the Codex interface to it, and reads public app-server events for agent activity. Type the task in Codex; approvals remain in its interface.
- In `[,]` → **Remote connection**, review the summary, press `[Enter]` to focus the form, save SSH connection metadata, choose the single active profile with `[A]`, and test key-based access to a VPS. **Check server** verifies GitHub-over-SSH and whether Codex CLI is installed and authenticated. **Configure server** opens an interactive SSH terminal with a step-by-step guide below it; you run each command yourself, and **Confirm configuration** rechecks the current requirement before advancing. Tuiminal does not create keys, install tools, connect accounts, copy projects, or start a remote agent automatically. **New Codex** remains local in this release.
...
- Follow recognized agents exclusively in **Agents**, separated into **Local • term** for agents observed through native, tmux, or external terminals and **Local • localhost** for integrated app-server sessions opened with Master Key `[A]`. Terminal agents appear first, with an animated loader while they work. Integrated Codex rows add a third line for code, command, plan, and tool activity; the current icon pulses from public app-server events. The portable icons `{}`, `>_`, `txt`, and `●` are always used, without requiring a patched font. When Codex is stopped, its first-line state represents it and all activity icons remain inactive. **Terminals** shows the remaining terminals. The original folders and splits are preserved, and a terminal returns to Terminals when its agent ends. With the Master Key open, the first nine visible agents and terminals show sequential `[1]`, `[2]`, … `[9]` labels in the same terminal-then-localhost order; type one to activate it.
- Open an integrated Codex session's sent-message history below its terminal with Master Key then `[S]`. Columns are ordered as elapsed time, status, message, image, audio, skill, and model. Status combines turn duration with `✓` for completion, `×` for failure or interruption, and `…` while running, such as `18m ✓`. Image, audio, and skill columns show `✓` only when that input was sent. Model configuration appears as `gpt-6-sol · medium · fast`; historical turns fall back to the selected thread's current configuration when app-server did not preserve it per turn, and show `—` only when neither source is available. Use `[J/K]`, `[↑/↓]`, or the mouse to select a row and `[Enter]` to open its detail. The expanded detail uses half of the terminal column and fills that area with theme-aware message, response, activity, and change cards. It provides `[M]` message, `[R]` final response and other public agent text, `[A]` complete public activity, and `[D]` that turn's changes. The change view separates the prompt-specific patch into file blocks, each with a visible path, change kind, statistics, syntax highlighting, line-number gutters, and addition/removal backgrounds; incomplete legacy patches use a colored line fallback instead of a blank preview. `[Esc]` steps back through section, detail, list, and terminal; `[X]` closes the panel. The in-memory view combines new public app-server input with the complete earlier history, loading every page of the selected thread; switching threads replaces the table. It exposes public final responses, commentary, reasoning summaries, plans, activities, and changes, but never private reasoning content. It is cleared on restart or close until the thread is loaded again. When Live Diff is open, it remains full-height to the right.
- In a split with two integrated Codex terminals, each terminal can keep its own sent-message history open; opening or closing one does not replace the other.
- Open and focus **Live Diff** for a recognized agent with Master Key then `[D]`; use the same shortcut again to restore its focus, and press `[X]` while focused to close it. It shares that agent's existing pane and shows the complete Git patch beside the terminal. The panel keeps a width fourteen columns narrower than its usual split; focusing the code widens only its preview by 30 columns while **Files** and **Info** keep their widths. Long diff lines stay unwrapped until the code is focused with `[Enter]`; then they wrap. In the code, `[J/K]` or `[↑/↓]` scroll one row, `[H/L]` or `[←/→]` scroll half a page, and `[Esc]` returns to the file list and automatic follow. Rows identify **New**, **Edit**, **Delete**, **Rename**, **Copy**, and **Type** changes. When one polling round detects changes in one or several files, each affected row receives a blue background that fades back to normal over two seconds. The file list uses `[J/K]` to select files and `[H/L]` or `[←/→]` to highlight a project. `[N]` shows or hides that project's files; a vivid background means visible, and a muted background means hidden. The Info summary puts the last changed project folder to the right of the totals on the same row; the Live Diff heading shows `Show auto: true/false`. The Files and Info boxes have no headings. **Watching** lists monitored projects as `/folder-name`, with separate backgrounds and balanced, left-aligned line breaks when needed. Git monitoring continues for hidden projects. Press `[A]` while navigating Live Diff, or click **Add project** alongside the other shortcuts, to open a searchable picker of nearby Git projects; related Git worktrees are discovered automatically. The preview follows the newest visible file and its latest changed hunk, while selecting an older file pauses automatic follow. `[Esc]` from file navigation returns focus to the terminal. The final snapshot remains after the agent exits until the panel is closed.
- In a section with two split terminals, each recognized agent can keep an independent Live Diff open; opening or closing the second does not replace the first. Live Diff normally uses about half of its selected terminal's pane while the other half and the sibling terminal remain visible. The stacked diff uses the complete pane width without the fourteen-column side-by-side reservation. It covers the selected pane only in very small layouts. Both processes remain mounted, and `[Esc]` or `[X]` restores the terminal without restarting either one.
- Pin the live sidebar with Master Key then `[B]` so Terminals and Agents stay visible without a right border while you move between Tuiminal tools. Use `[L]` from the action menu to focus it, move continuously through folder headings, Terminals and Agents with `[↑/↓]` or `[J/K]`, and press `[Enter]` to toggle a folder or open the highlighted item. Focus is marked by a fast light sweep across the sidebar background. Inside tmux, Tuiminal also keeps a marked left sidebar split in each window of the current server: after selecting that split with normal tmux navigation, its direct navigation works immediately, and it remains clickable while the neighboring terminal has focus. Folding a folder keeps that sidebar split selected. Selecting a terminal or agent from any pinned split always returns to Tuiminal and opens it in the Terminal workspace. If that pane is not open there yet, Tuiminal creates its mirror and lists it under Agents or Terminals as appropriate. Its Master Key opens the same actions and can run non-dialog actions without leaving that window. Unpinning removes only those helper splits and restores the previous tmux mouse setting.
- Typing `exit` in an interactive shell closes that terminal and removes it from Terminals; completed custom commands keep their final output available for inspection and restart.
...
open a centered modal whose actions are shown as a vertical list with two lines per
item: bracketed shortcut and name, then a short description. The modal also shows up
to six conversations from the local Codex `/resume` list for the current project.
Press an action key or click its control; select a conversation to resume it in the
official Codex TUI in a new section. Press `[/]` to focus the action/conversation filter.
`[Esc]` first leaves the filter, then closes the modal and restores terminal focus.
Global shortcut tokens in the top bar stay muted until the Master Key is active;
they then turn blue together with the temporary sidebar keys, without moving its layout.
Repeating the Master Key sends its literal control byte to the process.

| After the Master Key | Action |
| --- | --- |
| `[N]` | New terminal in a new section |
| `[A]` | Open the Codex interface connected to app-server in a new section |
| `[V]` / `[H]` | Choose a new shell or existing agent, then split right / below |
| `[S]` | Open or focus sent-message history for integrated Codex |
| `[M]` | Choose the sidebar or another visible box with arrows or `[H/J/K/L]`, then focus it with `[Enter]` |
...
| `[L]` | Focus the sidebar |
| `[E]` | Rename terminal |
| `[D]` | Open or focus Live Diff for a recognized agent |
| `[X]` | Close terminal |
| `[,]` | Open settings |
| `[Q]` | Quit Tuiminal |
...
running agent under **Local • term** or **Local • localhost**, with its status aligned to the right and task title below, falling
...
Newly detected lines keep a blue background while they remain in the patch, even
when a changed file is first previewed later. Blue lines keep a looping shimmer,
later edits add more blue lines, and a minimal blank row separates adjacent hunks.
...
| `bun run benchmark` | Measure offline response times for all five tools; see the [benchmark guide](./docs/performance-benchmarks.md) |
| `bun run benchmark:all` | Run the maintained service, native UI, and startup suites into one JSON report |
| `bun run benchmark:database:drivers` | Measure SQLite, MySQL, MariaDB, and PostgreSQL using disposable Docker containers |
