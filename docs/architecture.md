...
Git usa um wrapper pequeno para `[1] Base` e `[2] PR`. Base preserva árvore de
arquivos, grafo de commits e diffs locais; PR é montado sob demanda e separa modelo
puro, transporte `gh`, sessões/cache, configuração YAML, acompanhamento de CI,
ações coordenadas, renderização segura e componentes de lista/prévia/modal. O
registro global conhece somente `GitViewer`, escopo de teclado e disposer público.
