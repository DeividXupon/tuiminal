...
type ReadyResult = {
  status: "ready"
  root: string
  profile: { sections: readonly { id: string }[] }
  section: { id: string }
  hasNextPage: boolean
  refreshSeconds: number
}
type RemoteSessionResult = ReadyResult | { status: "config-error" } | { status: "requirements" }

type DashboardFailure =
  | { status: "authentication"; host: string }
  | { status: "error"; kind: string; error: string }

export type GitRemoteDashboardState<Result extends RemoteSessionResult> =
  | { status: "demo" }
  | { status: "idle" }
  | { status: "loading" }
  | DashboardFailure
...
type RemoteSession<Result extends RemoteSessionResult> = {
  loadSection: (
    root: string,
    sectionId?: string,
  ...
  refreshSections: (
    ...
    queryOverride: string | null,
    refreshAccountScope?: boolean,
  ) => Promise<Result>
  loadNextPage: (root: string, sectionId?: string, queryOverride?: string | null) => Promise<Result>
  cancelActiveLoad: () => void
  dispose: () => void
}

export function gitDashboardTransportFromEnvironment(): GhTransportOptions {
  const executable = process.env.TUIMINAL_GH_EXECUTABLE?.trim()
  return executable ? { executable } : {}
}

function dashboardError(error: unknown): DashboardFailure {
...
/** Shared PR/Issue dashboard request lifecycle; sessions retain all data rules and caches. */
export function useGitRemoteDashboard<Result extends RemoteSessionResult>({
  active,
  sectionId,
  queryOverride,
  ...
  demo: boolean
  source: string
  createSession: () => RemoteSession<Result>
}) {
  const [session] = useState(createSession)
  const [state, setState] = useState<GitRemoteDashboardState<Result>>(() =>
  ...
  const refreshSeconds = state.status === "ready" ? (state as ReadyResult).refreshSeconds : null
  ...
  const refresh = useCallback(async () => {
    ...
    setRefreshing(true)
    setRefreshError("")
    const ready = state as ReadyResult
    try {
      const result = await session.refreshSections(
        ready.root,
        ready.profile.sections.map((section) => section.id),
        sectionId,
        queryOverride,
      )
      if (generation === generationRef.current) setState(result)
  ...
  const refreshActive = useCallback(async () => {
    ...
    try {
      const result = await session.refreshSections(
        ready.root,
        queryOverride ? [] : [ready.section.id],
        ready.section.id,
        queryOverride,
        false,
      )
  ...
  const loadMore = useCallback(async () => {
    if (
      demo ||
      state.status !== "ready" ||
      !(state as ReadyResult).hasNextPage ||
    ...
    setRefreshError("")
    try {
      const result = await session.loadNextPage(
        (state as ReadyResult).root,
        sectionId,
        queryOverride,
      )
  ...
  useNotificationFromValue(refreshError, { source, kind: "error" })
