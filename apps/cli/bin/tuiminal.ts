...
if (selectedTool === "http" && args[1] === "run") {
  const { runHttpHeadless } = await import("@xupon/tuiminal-feature-http/cli/run")
  process.exit(await runHttpHeadless(args.slice(2)))
}
if (selectedTool === "http" && args[1] === "import") {
  const { importHttpCollectionCli } = await import("@xupon/tuiminal-feature-http/cli/import")
